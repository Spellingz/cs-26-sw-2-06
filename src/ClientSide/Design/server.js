...
function visualizeChange(wallIsHorizontal, wallConvertedIndex, result) {
    ...
    if (wallIsHorizontal) 
    {
        ...
        yPos = Math.floor(wallConvertedIndex / (stored.width-1)) * tileSize + lineWidth * 0.25;
    }
    else
    {
        xPos = (Math.floor(wallConvertedIndex / (stored.height-1))) * tileSize + lineWidth * 0.25;
    ...
    ctx.moveTo(xPos,yPos)
    ctx.lineTo(
            (wallIsHorizontal) ? xPos : (xPos + tileSize + lineWidth * 0.5),
            (wallIsHorizontal) ? (yPos + tileSize + lineWidth * 0.5) : yPos
    )
